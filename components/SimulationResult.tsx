...
export default function SimulationResult({ result }) {
  ...
  const evaluationResult = result.result?.EvaluationResult;
  const isSuccess = evaluationResult !== undefined;

  if (isSuccess) {
    const redeemers = Object.entries(evaluationResult);
    return (
      <div className="mt-4 p-4 rounded-lg bg-green-900/50 border border-green-400">
        ...
        <div className="mt-2 space-y-2 pl-8">
          {redeemers.map(([key, value]: [string, any]) => (
